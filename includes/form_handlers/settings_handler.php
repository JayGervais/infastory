...
if(isset($_POST['update_details'])) {
	...
	if($matched_user == "" || $matched_user == $userLoggedIn) {
		...
		$query = mysqli_query($con, "UPDATE users SET first_name='$first_name', last_name='$last_name', email='$email', description='$description' WHERE username='$userLoggedIn'");
	}
	else 
	...
	if(md5($old_password) == $db_password) {
		...
		if($new_password_1 == $new_password_2) {
			...
			if(strlen($new_password_1) <= 4) {
				$password_message = "Your password must be longer than 4 characters.<br><br>";
			}
			else {
